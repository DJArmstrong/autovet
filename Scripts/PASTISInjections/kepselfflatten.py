    ...
    if transitcut:
        if transitcut > 1:
            ...
            for cutidx in range(transitcut):
                timecut, fluxcut, errcut, qualitycut = CutTransits(timecut,fluxcut,errcut,
                										qualitycut,tc_t0[cutidx],
                										tc_per[cutidx],tc_tdur[cutidx])
        else:
            timecut, fluxcut, errcut, qualitycut = CutTransits(lc[:,0]+t0,lc[:,1],
            										lc[:,2],lc[:,3],tc_t0,tc_per,tc_tdur)
