...
class Candidate(object):
    ...
    '''
    def __init__(	self,id,filepath,observatory='NGTS',field_dic=None,label=-10,
        ...
        observatory -- 	Source of candidate. Accepted values are: [NGTS,Kepler,K2,'TESS']
        ...
        candidate_data   -- Details of candidate transit. Should be a dict containing 
        					the keys 'per', 't0' and 'tdur' (planet period, epoch and 
        					transit duration, all in days). If not filled, certain 
        					features may not work. Candidate will be ignored in 
    ...
    def NGTSload(self):
        ...
        nancut = np.isnan(	dic['HJD']) | np.isnan(dic['SYSREM_FLUX3']) | 
        					np.isnan(dic['FLUX3_ERR']) | np.isinf(dic['HJD']) | 
        					np.isinf(dic['SYSREM_FLUX3']) | np.isinf(dic['FLUX3_ERR']) | 
        					(dic['SYSREM_FLUX3']==0)
