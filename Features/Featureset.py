...
class Featureset(object):
    ...
    def __init__(self,Candidate,useflatten=False,testplots=False):
        ...
        testplots	-- 	bool
        				Plot diagnostic plots (only for some features)
        """
        self.features = {}
        ...
        self.fit_initialguess = np.array([self.target.candidate_data['per'],
        								  self.target.candidate_data['t0'],10.,0.005])
        self.trapfit_initialguess = np.array([self.target.candidate_data['t0'],
        									  self.target.candidate_data['tdur']*0.9/self.target.candidate_data['per'],
        									  self.target.candidate_data['tdur']/self.target.candidate_data['per'],0.005])
    ...
    def CalcFeatures(self,featuredict=None):
        ...
        Arguments:
        featuredict -- 	dict 
        				Features to calculate. {} to use observatory defaults. 
        				Feature name should be key, value should be necessary arguments 
        				(will often be empty list)
        """
        ...
        for featurename in featuredict.keys():
            ...
            if flag:
                ...
                #any errors lead to populating the feature with -10. 
                #Very catch-all, but worth it for the large numbers this is used on.
    ...
    def Writeout(self,activefeatures):
        '''
        Creates readable array of features
        
        Arguments:
        activefeatures	--	list
        					features to read out (must be pre-calculated)
        
        Returns:
        Target_IDs		--	array of target IDs
        Target_Labels	--	array of target labels
        Target_Features	--	list of feature arrays
        '''
    ...
    def ReadIn(self,featurearray,featurenames):
    	'''
    	Incorporates feature data into self.features
    	'''
    ...
    def TSFresh(self,args):
        ...
        The number of features TSFresh returns seems to be somewhat variable. 
        Care may have to be taken with the rarer ones.
        Set up to run on one candidate, but could be changed to 
        run on an entire field/set at once.
        """
        import tsfresh
        ...
        if self.tsfresh is None:  #will only calculate set the first time
            ...
            self.tsfresh = extract_features(tsinput,column_id='index',
            								column_sort='time',column_value='flux')

        return self.tsfresh[args[0]][1.0]

    def SOM_Theta1(self,args):
        """
        SOM Theta1 statistic from Armstrong et al 2017. Uses pre-trained SOMs, 
        produces statistic ranking candidate's transit shape.
        """
        if self.useflatten:
        ...
        if self.target.obs == 'Kepler':
            if self.som is None:
                self.som = TSOM.TSOM.LoadSOM(os.path.join(self.__somlocation__,
                							 'snrcut_30_lr01_300_20_20_bin50.txt'),20,20,
                							  50,0.1)
                lc_sominput = np.array([lc['time'],lc['flux'],lc['error']]).T
                self.SOMarray,self.SOMerror = TSOM.TSOM.PrepareOneLightcurve(lc_sominput,
                							  self.target.candidate_data['per'],
                							  self.target.candidate_data['t0'],
                							  self.target.candidate_data['tdur'],
                							  nbins=50)
            planet_prob = TSOM.TSOM.ClassifyPlanet(self.SOMarray,self.SOMerror,
            			  missionflag=0,case=1,flocation=self.__somlocation__)
            
        elif self.target.obs == 'K2':
            if self.som is None:
                self.som = TSOM.TSOM.LoadSOM(os.path.join(self.__somlocation__,
                		   					 'k2all_lr01_500_8_8_bin20.txt'),8,8,
                		   					  20,0.1)
                lc_sominput = np.array([lc['time'],lc['flux'],lc['error']]).T
                self.SOMarray,self.SOMerror = TSOM.TSOM.PrepareOneLightcurve(lc_sominput,
                							  self.target.candidate_data['per'],
                							  ...
                							  nbins=20)
            planet_prob = TSOM.TSOM.ClassifyPlanet(self.SOMarray,self.SOMerror,
            			  missionflag=1,case=1,flocation=self.__somlocation__)
            
        elif (self.target.obs == 'NGTS') or (self.target.obs == 'NGTS_synth'):
            if self.som is None:
                self.som = TSOM.TSOM.LoadSOM(os.path.join(self.__somlocation__,
                							 'NGTSOM_20_20_100_0.1.txt'),20,20,
                							  20,0.1)
                lc_sominput = np.array([lc['time'],lc['flux'],lc['error']]).T
                self.SOMarray,self.SOMerror = TSOM.TSOM.PrepareOneLightcurve(lc_sominput,
                							  self.target.candidate_data['per'],
                							  ...
                							  nbins=20)
            planet_prob = TSOM.TSOM.ClassifyPlanet(self.SOMarray,self.SOMerror,
            			 missionflag=2,case=1,flocation=self.__somlocation__)
        if type(planet_prob)=='numpy.ndarray':  
            #the faked extra transit from a different SOM function was present
    ...
    def SOM_Theta2(self,args):
        """
        SOM Theta2 statistic from Armstrong et al 2017. Uses pre-trained SOMs, 
        produces statistic ranking candidate's transit shape.
        """
        if self.useflatten:
            lc = self.target.lightcurve_f
        ...
        if self.target.obs == 'Kepler':
            if self.som is None:
                ...
                self.SOMarray,self.SOMerror = TSOM.TSOM.PrepareOneLightcurve(lc_sominput,
                							  ...
                							  nbins=50)
            planet_prob = TSOM.TSOM.ClassifyPlanet(self.SOMarray,self.SOMerror,
            			  missionflag=0,case=2,flocation=self.__somlocation__)
            ...
            if self.som is None:
                ...
                self.SOMarray,self.SOMerror = TSOM.TSOM.PrepareOneLightcurve(lc_sominput,
                							  ...
                							  nbins=20)
            planet_prob = TSOM.TSOM.ClassifyPlanet(self.SOMarray,self.SOMerror,
            			  missionflag=1,case=2,flocation=self.__somlocation__)
            ...
            if self.som is None:
                ...
                self.SOMarray,self.SOMerror = TSOM.TSOM.PrepareOneLightcurve(lc_sominput,
                							  ...
                							  nbins=20)
            planet_prob = TSOM.TSOM.ClassifyPlanet(self.SOMarray,self.SOMerror,
            			  missionflag=2,case=2,flocation=self.__somlocation__)
        if type(planet_prob)=='numpy.ndarray':  
            #the faked extra transit from a different SOM function was present
            planet_prob = planet_prob[0]
        if self.testplots:
    ...
    def LSPeriod(self,args):
        ...
        peak_number	--	which LS peak to extract (starts at 0 for largest peak). 
        				If multiple, should call this function with the largest 
        				peak_number desired first.
        ...
        if self.periodls is None: #checks it hasn't been defined before
            if self.target.obs == 'K2':
                self.periodls = PeriodLS.PeriodLS(self.target.lightcurve,
                								 observatory=self.target.obs)        
            else:
                self.periodls = PeriodLS.PeriodLS(self.target.lightcurve,
                								 observatory=self.target.obs,
                								 removethruster=False,removecadence=False)
        output = []
        for peak_number in args:
            self.periodls.fit(peak_number) 
            #will only run fit if peak_number greater than any previously run.
            #However, will run fit from start (fit once, remove peak, refit...) otherwise
    ...
    def LSAmp(self,args): 
        ...
        Returns:
        [ampratio1,ampratio2,...] --	ratio of peak peak_number amplitude to 
    ...
    def EBPeriod(self,args):
        ...
        Uses the LSPeriod0 feature calculation as the base period.
        Returns either the original period or double.    
        """
        lc = self.target.lightcurve
    ...
    def P2P_98perc(self,args):
        """
        98th percentile of point-to-point difference across flux. 
        Does not take account of gaps.
        """
        return np.percentile(np.abs(np.diff(self.target.lightcurve['flux'])),98)

    def F8(self,args):
        """
        'Flicker' on 8-hour timescale. 
        Calculated through std around an 8-hour moving average.
        """
        if self.useflatten:
    ...
    def MaxSecDepth(self,args):
        ...
        Scans a box of width the transit duration over phases 0.3 to 0.7. 
        Returns maximum significance of box relative to local phasecurve, 
        normalised by point errors.
        """
    ...
    def LSPhase_amp(self,args):
        '''
        Uses EBPeriod period.
        Amplitude of binned phase-folded lightcurve
        '''
        lc = self.target.lightcurve
    ...
    def LSPhase_p2pmean(self,args):
        '''
        Uses EBPeriod period.
        Point-to-point mean of phase folded lightcurve.
        '''
        lc = self.target.lightcurve
        if 'EBPeriod' not in self.features.keys():
            self.features['EBPeriod'] = self.EBPeriod([])
    ...
    def LSPhase_p2pmax(self,args):
        '''
        Uses EBPeriod period.
        Point-to-point max of phase folded lightcurve.
        '''        
        lc = self.target.lightcurve
        if 'EBPeriod' not in self.features.keys():
            self.features['EBPeriod'] = self.EBPeriod([])
    ...
    def Fit_period(self,args):  #change to useflatten option?
        if self.transitfit is None:
            self.transitfit = TransitFit.TransitFit(self.target.lightcurve,
            				  self.fit_initialguess,self.target.exp_time,sfactor=7)
    ...
    def Fit_duration(self,args):  
        #assumes circular at the moment, as fit doesn't allow for varying e
        if self.transitfit is None:
            self.transitfit = TransitFit.TransitFit(self.target.lightcurve,
            				  self.fit_initialguess,self.target.exp_time,sfactor=7)
        #b = self.transitfit.params[2]*np.cos(self.transitfit.params[4])
        b = 0
        tdur = self.transitfit.params[0]/math.pi * np.arcsin(np.sqrt(
        		(1+self.transitfit.params[3])**2-b**2)/(self.transitfit.params[2]*1.))    
        return tdur
    ...
    def Fit_depthSNR(self,args): #errors are currently shoddy, so don't trust this feature
        if self.transitfit is None:
            self.transitfit = TransitFit.TransitFit(self.target.lightcurve,
            				  self.fit_initialguess,self.target.exp_time,sfactor=7)
    ...
    def Even_Fit_aovrstar(self,args):
        ...
        if self.evenlc is None:
            self.evenlc = utils.SplitOddEven(self.target.lightcurve,
            			  self.target.candidate_data['per'],
            			  self.target.candidate_data['t0'],'even')
        if self.eventransitfit is None:
            self.eventransitfit = TransitFit.TransitFit(self.evenlc,
            				 	  self.fit_initialguess,self.target.exp_time,sfactor=7,
            				 	  fixper=self.transitfit.params[0],
            				 	  fixt0=self.transitfit.params[1])
    ...
    def Odd_Fit_aovrstar(self,args):
        ...
        if self.oddlc is None:
            self.oddlc = utils.SplitOddEven(self.target.lightcurve,
            			  self.target.candidate_data['per'],
            			  self.target.candidate_data['t0'],'odd')
        if self.oddtransitfit is None:
            self.oddtransitfit = TransitFit.TransitFit(self.oddlc,
            					  self.fit_initialguess,self.target.exp_time,sfactor=7,
            					  fixper=self.transitfit.params[0],
            					  fixt0=self.transitfit.params[1])
    ...
    def RPlanet(self,args):
        ...
        rsun = 6957000000.
        rearth = 6371000.
        return self.transitfit.params[3]*self.target.stellar_radius* rsun/rearth
           
    def TransitSNR(self,args):
        '''
        Transit depth from binned phase-fold, normalised by std of out of transit bins.
        '''
        per = self.target.candidate_data['per']    
        tdur = self.target.candidate_data['tdur']
        t0 = self.target.candidate_data['t0']
        ...
        #0.8 is to avoid ingress and egress
        binnedlc = utils.BinTransitDuration(lc,per,t0,tdur*0.8) 
        binnedlc = binnedlc[binnedlc[:,1]>=0]
        transitbin = binnedlc[0,1]
        noise = np.std(binnedlc[2:-2,1]) #avoids duration bins containing in/egress
        if np.isnan(noise):
            return -10
        else:
    ...
    def SingleTransitEvidence(self,args):
        '''
        Median TransitSNR calculated for individual transits.
        '''
        per = self.target.candidate_data['per']    
        tdur = self.target.candidate_data['tdur']
        t0 = self.target.candidate_data['t0']
    ...
    def RMS_TDur(self,args):
        """
        RMS on transit duration timescale. 
        Calculated through std around a transit duration moving average.
        """
        if self.useflatten:
            lc = self.target.lightcurve_f  #uses flattened lightcurve
        else:
        ...
        if self.trapfit is None:
            self.trapfit = TransitFit.TransitFit(self.target.lightcurve,
            			   self.trapfit_initialguess,self.target.exp_time,sfactor=7,
            			   fittype='trap',fixper=self.target.candidate_data['per'])            
    ...
    def Even_Trapfit_t23phase(self,args):
        ...
        if self.eventrapfit is None:
            self.eventrapfit = TransitFit.TransitFit(self.evenlc,
            				   self.trapfit_initialguess,self.target.exp_time,sfactor=7,
            				   fittype='trap',fixper=self.target.candidate_data['per'],
            				   fixt0=self.trapfit.params[0])
    ...
    def Odd_Trapfit_t23phase(self,args):
        ...
        if self.oddtrapfit is None:
            self.oddtrapfit = TransitFit.TransitFit(self.oddlc,
            				  self.trapfit_initialguess,self.target.exp_time,sfactor=7,
            				  fittype='trap',fixper=self.target.candidate_data['per'],
            				  fixt0=self.trapfit.params[0])
    ...
    def PointDensity_ingress(self,args):
        '''
        Density of points in ingress/egress relative to average point 
        density of lightcurve.
    ...
    def missingDataFlag(self,args):
        '''
        Fraction of empty tdur/5 width bins within 5 transit durations 
        centred on transit, in phase
        '''
        per = self.target.candidate_data['per']
